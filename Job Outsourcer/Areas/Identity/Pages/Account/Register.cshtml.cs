...
namespace Job_Outsourcer.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (role == StaticDetails.Partner)
                    {

                        await _userManager.AddToRoleAsync(user, StaticDetails.Partner);
                        await _signInManager.SignInAsync(user, isPersistent: false);

                        PartnerRights partnerRightsDb = new Models.PartnerRights();
                        partnerRightsDb.UserId = user.Id;
                        partnerRightsDb.JobTypeId = 4;
                        _unitOfWork.PartnerRights.Add(partnerRightsDb);
                        _unitOfWork.Save();

                        return LocalRedirect("/Partner/Home/Index");
                    ...
                    else
                    {
                        ...
                        else
                        {
                                ...
                                await _signInManager.SignInAsync(user, isPersistent: false);
                                return LocalRedirect("/Customer/Jobs/Index");
